...
get_compose_type(){
    if ! podman compose > /dev/null 2>&1; then
        if podman compose version --short | grep -e "^1.5." -e "^v1.5." > /dev/null 2>&1; then
    ...
    elif podman-compose > /dev/null 2>&1; rc=$?; [[ $rc -eq 0 || $rc -eq 255 ]]; then
    if ! [[ $(alias podman-compose 2> /dev/null) ]] ; then
        if podman-compose version --short | grep "^1.5." > /dev/null 2>&1; then
