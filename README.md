...
Results so far:
* Testing is being done with IPv6 disabled. The ipv6nat container keeps crashing anyway.
* All the containers (bar ipv6nat) come up and communicate with each other, albeit briefly.
* Stability is flakey, the GUI constantly loses connectivity to the backend, containers keep going down and up.
  * I imagine this is due to the health checks and connectivity due to DNS, but it could be something else.
